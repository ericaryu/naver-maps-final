...
export default async function handler(req, res) {
  ...
  try {
    // 1) Geocode
    const geoResp = await axios.get("https://dapi.kakao.com/v2/local/search/keyword.json", {
    ...
    // 2) Kakao restrooms
    const kakaoData = await axios.get("https://dapi.kakao.com/v2/local/search/keyword.json", {
      headers: { Authorization: `KakaoAK ${KAKAO_API_KEY}` },
    ...
    // 3) Public restrooms
    const publicResp = await axios.get("https://api.odcloud.kr/api/15044453/v1/uddi:3f0b1632-3ac0-4bc2-97ef-8fa94fcfb23c", {
      params: { page: 1, perPage: 1000, serviceKey: ODCLOUD_API_KEY }
    });
    const publicList = publicResp.data.data.map(item => ({
      source: "public",
      type: "closest",
